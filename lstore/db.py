...
class Database():
    ...
    def grab_table(self, name, path):
        ...
        meta_file.close()
        self.tables[name] = Table(name, meta_page.read(0), meta_page.read(1))
        rid_path = os.path.join(path, "RID.bin")
        file_size = os.path.getsize(rid_path)
        self.tables.load_col( rid_path, file_size)
        for i in range(0, self.tables.numcolumns): 
            self.tables.load_col( os.path.join(path,  str(i)+ ".bin"), file_size, i)
